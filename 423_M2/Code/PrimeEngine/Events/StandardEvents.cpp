...
int Event_CREATE_MESH::l_Construct(lua_State* luaVM)
{
	...
	PE::GameContext *pContext = (PE::GameContext*)(lua_touserdata(luaVM, -17));
	
	Event_CREATE_MESH *pEvt = new(h) Event_CREATE_MESH(pContext->m_gameThreadThreadOwnershipMask);

	const char* name = lua_tostring(luaVM, -16);
	const char* package = lua_tostring(luaVM, -15);
	const char* type = "";
	
	float positionFactor = 1.0f / 100.0f;
	Vector3 pos, u, v, n;
	pos.m_x = (float)lua_tonumber(luaVM, -14) * positionFactor;
	pos.m_y = (float)lua_tonumber(luaVM, -13) * positionFactor;
	pos.m_z = (float)lua_tonumber(luaVM, -12) * positionFactor;

	u.m_x = (float)lua_tonumber(luaVM, -11);
	u.m_y = (float)lua_tonumber(luaVM, -10);
	u.m_z = (float)lua_tonumber(luaVM, -9);

	v.m_x = (float)lua_tonumber(luaVM, -8);
	v.m_y = (float)lua_tonumber(luaVM, -7);
	v.m_z = (float)lua_tonumber(luaVM, -6);

	n.m_x = (float)lua_tonumber(luaVM, -5);
	n.m_y = (float)lua_tonumber(luaVM, -4);
	n.m_z = (float)lua_tonumber(luaVM, -3);

	if (!lua_isnil(luaVM, -2))
	{
		type = lua_tostring(luaVM, -2);
	}
	...
	StringOps::writeToString(type, pEvt->m_meshType, 255);
	lua_pop(luaVM, 16); //Second arg is a count of how many to pop
...
//UDP Events
//////////////////////////////////////////////////////////////////////////
PE_IMPLEMENT_CLASS1(Event_CLIENT_SERVER_UDP_ACK, Event);
Event_CLIENT_SERVER_UDP_ACK::Event_CLIENT_SERVER_UDP_ACK(PE::GameContext &context)
	: Networkable(context, this)
	, m_port(-1)
{
...
Event_CLIENT_SERVER_UDP_ACK::~Event_CLIENT_SERVER_UDP_ACK()
{
	assert("Quick test");
}


void *Event_CLIENT_SERVER_UDP_ACK::FactoryConstruct(PE::GameContext& context, PE::MemoryArena arena)
{
	Event_CLIENT_SERVER_UDP_ACK *pEvt = new (arena) Event_CLIENT_SERVER_UDP_ACK(context);
	return pEvt;
}

int Event_CLIENT_SERVER_UDP_ACK::packCreationData(char *pDataStream)
{
	int written = 0;
	written += PE::Components::StreamManager::WriteInt32(m_port, pDataStream);
	return written;
}

int Event_CLIENT_SERVER_UDP_ACK::constructFromStream(char *pDataStream)
{
	int read = 0;
	read += PE::Components::StreamManager::ReadInt32(&pDataStream[read], m_port);
	return read;
}

PE_IMPLEMENT_CLASS1(Event_SERVER_CLIENT_UDP_ACK, Event);
Event_SERVER_CLIENT_UDP_ACK::Event_SERVER_CLIENT_UDP_ACK(PE::GameContext &context)
	: Networkable(context, this)
	, m_port(-1)
{
...
Event_SERVER_CLIENT_UDP_ACK::~Event_SERVER_CLIENT_UDP_ACK()
{
	assert("Quick test");
}


void *Event_SERVER_CLIENT_UDP_ACK::FactoryConstruct(PE::GameContext& context, PE::MemoryArena arena)
{
	Event_SERVER_CLIENT_UDP_ACK *pEvt = new (arena) Event_SERVER_CLIENT_UDP_ACK(context);
	return pEvt;
}

int Event_SERVER_CLIENT_UDP_ACK::packCreationData(char *pDataStream)
{
	int written = 0;
	written += PE::Components::StreamManager::WriteInt32(m_port, pDataStream);
	return written;
...
int Event_SERVER_CLIENT_UDP_ACK::constructFromStream(char *pDataStream)
{
	int read = 0;
	read += PE::Components::StreamManager::ReadInt32(&pDataStream[read], m_port);
	return read;

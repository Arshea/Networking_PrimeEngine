...
struct Event_CREATE_MESH : public Event {
	...
	Event_CREATE_MESH(int &threadOwnershipMask) : hasCustomOrientation(false), m_threadOwnershipMask(threadOwnershipMask)
	{
		...
		StringOps::writeToString("", m_meshType, 255);
	}
	...
	PEUUID m_peuuid;
	int &m_threadOwnershipMask;
	char m_isTargetName[255];
	char m_getsTargetName[255];
};
...
struct Event_CLIENT_SERVER_UDP_ACK : public Event, public Networkable {
	PE_DECLARE_CLASS(Event_CLIENT_SERVER_UDP_ACK);
	PE_DECLARE_NETWORKABLE_CLASS

		Event_CLIENT_SERVER_UDP_ACK(PE::GameContext &context);
	virtual ~Event_CLIENT_SERVER_UDP_ACK();
	...
	PrimitiveTypes::Int32 m_port;
};

struct Event_SERVER_CLIENT_UDP_ACK : public Event, public Networkable {
	PE_DECLARE_CLASS(Event_SERVER_CLIENT_UDP_ACK);
	PE_DECLARE_NETWORKABLE_CLASS

		Event_SERVER_CLIENT_UDP_ACK(PE::GameContext &context);
	virtual ~Event_SERVER_CLIENT_UDP_ACK();

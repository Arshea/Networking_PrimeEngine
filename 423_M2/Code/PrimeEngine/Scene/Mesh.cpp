...
EPEVertexFormat Mesh::updateGeoFromMeshCPU_needsRC(MeshCPU &mcpu, int &threadOwnershipMask)
{
	...
	//					DOING A THING					 //
	///////////////////////////////////////////////////////
	/*PositionBufferCPU *pvbcpu = m_hPositionBufferCPU.getObject<PositionBufferCPU>();
	...
	/*SceneNode *parent = getFirstParentByTypePtr<SceneNode>();
	Matrix4x4 base = parent->m_base;
	// Add base to min and max points; store in imrod mesh
	 min[0] = min[0] + base.getPos().getX();
	 min[1] = min[1] + base.getPos().getY();
	 min[2] = min[2] + base.getPos().getZ();
	 max[0] = max[0] + base.getPos().getX();
	 max[1] = max[1] + base.getPos().getY();
	 max[2] = max[2] + base.getPos().getZ();*/
	...
	for (int i = 0; i < 24; i++) {
		linepts[2 * i] = * vertices[verticesToAdd[i]];
		linepts[2 * i + 1] = color;
	}

	DebugRenderer::Instance()->createLineMesh(false, Matrix4x4(), &linepts[0].m_x, 24, 1);*/
	return res;
}

...
struct Matrix4x4 {
	...
	*/
	...
	void turnTo(Vector3 targetPos) // Never ever trust Artem's code. Only works on one axis. If he'd commented that like a regular person I'd have saved hours.
	{
		Vector3 forward = getN();
		forward.m_y = 0.0f;
		forward.normalize();
	...
	void turnToVertical(Vector3 targetPos) {
		Vector3 forward = getN();
		forward.m_x = 0.0f;
		forward.normalize();
		Vector3 newForward = targetPos - getPos();
		...
		PrimitiveTypes::Float32 signOfSinAlpha = forward.crossProduct(newForward).m_x;
		...
		if (signOfSinAlpha > 0)
		{
			// need to turn down by alpha
			turnDown(alpha);
		}
		else
		{
			// need to turn up by alpha
			turnUp(alpha);
		}
	}

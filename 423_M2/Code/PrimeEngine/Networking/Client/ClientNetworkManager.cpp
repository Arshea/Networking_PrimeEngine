...
void ClientNetworkManager::clientConnectToUDPServer(const char *strAddr, int port)
{
	...
	{
		const char* error = inet_trybind(&sock, "0.0.0.0", port);
		PEINFO("PE: Client connected to %s:%d\n", strAddr, port);
		created = true;
		m_netContextLock.lock();

		//setup the address datas
		m_recvaddr.sin_family = AF_INET;
		m_recvaddr.sin_port = htons(port);
		std::uint32_t ip;
		inet_pton(AF_INET, strAddr, &ip);
		m_recvaddr.sin_addr.s_addr = ip;
		//memset(m_recvaddr.sin_zero, 0, sizeof(m_recvaddr.sin_zero));

		m_sendtoaddr.sin_family = AF_INET;
		m_sendtoaddr.sin_port = htons(1660);
		m_sendtoaddr.sin_addr.s_addr = ip;
		//memset(m_sendtoaddr.sin_zero, 0, sizeof(m_recvaddr.sin_zero));
		...
		// use an event to communicate with the server
		PE::Handle evtHandle("Event_CLIENT_SERVER_UDP_ACK", sizeof(Event_CLIENT_SERVER_UDP_ACK));
		PE::Events::Event_CLIENT_SERVER_UDP_ACK *evt = new (evtHandle)Event_CLIENT_SERVER_UDP_ACK(*m_pContext);
		evt->m_port = port;
		m_netContext.getEventManager()->scheduleEvent(evt, (Networkable*)m_pContext->getGameObjectManager(), false);
...
void ClientNetworkManager::createNetworkConnectionContext(t_socket sock, PE::NetworkContext *pNetContext)
{
	...
	pNetContext->getConnectionManager()->initializeConnected(sock, m_recvaddr, m_sendtoaddr);
...
void ClientNetworkManager::addDefaultComponents()
{
	...
	PE_REGISTER_EVENT_HANDLER(PE::Events::Event_SERVER_CLIENT_UDP_ACK, ClientNetworkManager::do_SERVER_CLIENT_UDP_ACK);
...
void ClientNetworkManager::do_SERVER_CLIENT_UDP_ACK(PE::Events::Event *pEvt)
{
...
void ClientNetworkManager::debugRender(int &threadOwnershipMask, float xoffset /* = 0*/, float yoffset /* = 0*/)
{
	...
	m_netContextLock.lock();
	m_netContext.getConnectionManager()->debugRender(threadOwnershipMask, xoffset - (20 * dx), yoffset + (2 * dy));
...
int ClientNetworkManager::l_clientConnectToUDPServer(lua_State *luaVM)
{
	lua_Number lPort = lua_tonumber(luaVM, -1);
	int port = (int)(lPort);

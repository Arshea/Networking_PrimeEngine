...
void ServerNetworkManager::addDefaultComponents()
{
	...
	PE_REGISTER_EVENT_HANDLER(PE::Events::Event_CLIENT_SERVER_UDP_ACK, ServerNetworkManager::do_CLIENT_SERVER_UDP_ACK);
...
void ServerNetworkManager::serverOpenUDPSocket()
{
	...
	while (!created)
	{
		...
		err = inet_trybind(&m_sock, "0.0.0.0", (unsigned short)(port)); // leaves socket non-blocking on default 1660
		numTries++;
		if (err) {
			if (numTries >= 10)
				break; // give up
	...
	if (created)
	{
		m_sockList[0] = m_sock;
		m_serverPort = port;
		//setup the address data
		m_recvaddr.sin_family = AF_INET;
		m_recvaddr.sin_port = htons(port);
		std::uint32_t ip;
		inet_pton(AF_INET, "127.0.0.1", &ip);
		m_recvaddr.sin_addr.s_addr = ip;

		m_sendaddr.sin_family = AF_INET;
		// NOTE THAT THE PORT IS SET IN do_CLIENT_SERVER_UDP_ACK
		m_sendaddr.sin_addr.s_addr = ip;
		...
		createNetworkConnectionContext(m_sock, clientIndex, &netContext);
		m_connectionsMutex.unlock();

		//will want to send this as a response to CLIENT_SERVER_UDP_ACK
	...
	{
		assert(!"Could not create server udp");
		return;
	}
...
void ServerNetworkManager::createNetworkConnectionContext(t_socket sock,  int clientId, PE::NetworkContext *pNetContext)
{
	...
	pNetContext->getConnectionManager()->initializeConnected(sock, m_recvaddr, m_sendaddr);
...
void ServerNetworkManager::do_CLIENT_SERVER_UDP_ACK(PE::Events::Event *pEvt)
{
	if (m_actualConnections != 0)
	{
		//do initialization for 2 connection
		m_connectionsMutex.lock();
		m_clientConnections.add(NetworkContext());
		int clientIndex = m_clientConnections.m_size - 1;
		NetworkContext &netContext = m_clientConnections[clientIndex];
	...
	m_actualConnections++; //make debug correct
	Event_CLIENT_SERVER_UDP_ACK *pTrueEvent = (Event_CLIENT_SERVER_UDP_ACK*)(pEvt);
	//set port as sent by the message
	m_sendaddr.sin_port = htons(pTrueEvent->m_port);

	//send ack message back
	PE::Events::Event_SERVER_CLIENT_CONNECTION_ACK evt(*m_pContext);
	evt.m_clientId = m_actualConnections - 1;
	NetworkContext &netContext = m_clientConnections[m_actualConnections - 1];

	//update sendaddr 
	netContext.getConnectionManager()->m_sendaddr = m_sendaddr;

	//send away to load the level
	netContext.getEventManager()->scheduleEvent(&evt, m_pContext->getGameObjectManager(), false);
...
void ServerNetworkManager::scheduleEventTo(PE::Networkable *pNetworkable, PE::Networkable *pNetworkableTarget, int targetClient)
{
	for (unsigned int i = 0; i < m_clientConnections.m_size; ++i)
	{
		if ((int)(i) != targetClient)
			continue;

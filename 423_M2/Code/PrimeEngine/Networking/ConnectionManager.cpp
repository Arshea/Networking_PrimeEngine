...
void ConnectionManager::initializeConnected(t_socket sock, sockaddr_in recvaddr, sockaddr_in sendaddr)
{
	m_sock = sock;
	m_recvaddr = recvaddr;
	m_sendaddr = sendaddr;
...
#else
				err = socket_sendto(&m_sock, &pPacket->m_data[total], step, &done, (SA *)&m_sendaddr, sizeof(m_sendaddr), tm);
#endif
			}
			else
			{
...
void ConnectionManager::receivePackets()
{
    ...
    while (true)
    {
		...
		err = socket_recvfrom(&m_sock, &m_buffer[m_bytesBuffered], PE_SOCKET_RECEIVE_BUFFER_SIZE - m_bytesBuffered, &got, (SA *)&m_recvaddr, &len, tm);
#endif
...
void ConnectionManager::debugRender(int &threadOwnershipMask, float xoffset /* = 0*/, float yoffset /* = 0*/) {
	sprintf(PEString::s_buf, "Packets Dropped: %d", num_packets_dropped);

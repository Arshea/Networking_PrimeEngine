...
namespace PE {
	...
	void MaterialSetCPU::createSetWithOneTexturedMaterialCubeMap(const char *textureFilename, const char *package, ESamplerState customSamplerState /* = SamplerState_Count*/)
	{
		m_materials.reset(1);
		MaterialCPU matCPU(*m_pContext, m_arena);
		matCPU.createMaterialWithColorTextureCubeMap(textureFilename, package, customSamplerState);
		m_materials.add(matCPU);
		prepareStats();
	}

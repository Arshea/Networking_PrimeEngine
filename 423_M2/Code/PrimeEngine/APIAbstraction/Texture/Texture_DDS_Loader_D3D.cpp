...
namespace PE {
		...
		TextureGPU::gfxLoadDDSTexture(char *filename, bool genMips /*= true*/)
	{
		...
		if (m_family == TextureFamily::COLOR_CUBE)
		{
			ID3D11ShaderResourceView* pShaderResourceView = NULL;
			D3D11_SHADER_RESOURCE_VIEW_DESC SRVDesc;

			//Describing each face of cube map
			...
			D3D11_SUBRESOURCE_DATA datas[6 * 10]; //6 faces * 10 mips
			for (uint32_t i = 0; i < dds.mips; i++)
			{
				for (int cubeMapIndex = 0; cubeMapIndex < 6; cubeMapIndex++)
				{
					datas[i * 6 + cubeMapIndex].pSysMem = dds.image[cubeMapIndex].pixels[i]; //if this doesn't work, try changing it to pixels[i]
					datas[i * 6 + cubeMapIndex].SysMemPitch = dds.components * w;
					datas[i * 6 + cubeMapIndex].SysMemSlicePitch = 0; // unused since 2d texture
				}
				w /= 2; h /= 2;
				if (w == 0) w = 1;
				if (h == 0) h = 1;
			...
			HRESULT hr = pDevice->CreateTexture2D(&desc, datas, &pTexture);
			if (hr != S_OK)
			{
				return false;
			}
			...
			hr = pDevice->CreateShaderResourceView(m_pTexture, &SRVDesc, &pShaderResourceView);
			if (hr != S_OK)
			{
				return false;
			}

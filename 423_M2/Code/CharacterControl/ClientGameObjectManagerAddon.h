...
struct ClientGameObjectManagerAddon : public GameObjectManagerAddon
{
	...
	//sent from server when one player has won to all losing players
	PE_DECLARE_IMPLEMENT_EVENT_HANDLER_WRAPPER(do_Lose);
	virtual void do_Lose(PE::Events::Event *pEvt);

	//sent from server when one player has won to all losing players
	PE_DECLARE_IMPLEMENT_EVENT_HANDLER_WRAPPER(do_Win);
	virtual void do_Win(PE::Events::Event *pEvt);

	//sent from server when one player moves to all other players
	...
	// Target search
	SoldierNPC *getTarget(const char *name);

...
// sent by movement state machine when it reaches current target
void SoldierNPCBehaviorSM::do_SoldierNPCMovementSM_Event_TARGET_REACHED(PE::Events::Event *pEvt)
{
	...
	if (m_state == PATROLLING_WAYPOINTS)
	{
		...
		if (pGameObjectManagerAddon)
		{
			...
			// Count for number of next waypoints set
			int nextWayPointCount = 0;
			for (int i = 0; i < 3; i++) {
				if (pWP && StringOps::length(pWP->m_nextWayPointName[i]) > 0) nextWayPointCount++;
				else i = 3; // break
			}

			if (nextWayPointCount > 0)
			{
				// Randomise WayPoint
				int wayPointChosen = rand() % nextWayPointCount;
				// have next waypoint to go to
				pWP = pGameObjectManagerAddon->getWayPoint(pWP->m_nextWayPointName[wayPointChosen]);
				if (pWP)
				{
			...
			else if(m_hasTarget) // NEW BUT DOESN'T WORK :[
			{
				OutputDebugStringA("Has a target!\n");
				OutputDebugStringA("The target name is ");
				OutputDebugStringA(m_getsTargetName);
				OutputDebugStringA("\n");
				ClientGameObjectManagerAddon *pGameObjectManagerAddon = (ClientGameObjectManagerAddon *)(m_pContext->get<CharacterControlContext>()->getGameObjectManagerAddon());
				if (pGameObjectManagerAddon)
				{
					// search for waypoint object
					OutputDebugStringA("Now looking for target ");
					OutputDebugStringA(m_getsTargetName);
					OutputDebugStringA("\n");


					SoldierNPC *pSol = pGameObjectManagerAddon->getTarget(m_getsTargetName);
					if (pSol)
					{
						OutputDebugStringA("Soldier with name exists!\n");
						m_state = SHOOTING;
						PE::Handle h("SoldierNPCMovementSM_Event_MOVE_TO", sizeof(SoldierNPCMovementSM_Event_MOVE_TO));
						Events::SoldierNPCMovementSM_Event_MOVE_TO *pEvt = new(h) SoldierNPCMovementSM_Event_MOVE_TO(pSol->m_pos.getPos());
